...
- **Onboarding**: after first sign-in, `POST /profile/complete` — patients supply DOB; doctors self-attest a `registrationNumber` and `phoneNumber` and are active immediately, no external verification step. Patients verify a doctor themselves via the registration number shown in the app.
...
Also loads `seed/institutions.sql` first — 8 real Trinidad & Tobago public hospitals (each with an Obstetrics & Gynaecology department), idempotent via `INSERT OR IGNORE` — so the doctor affiliation picker has realistic choices beyond the one test institution below.

Seeds four doctors and five patients. Patients A–D each get roughly a month of dated readings (a handful of hand-crafted "anchor" rows for specific correctness cases, plus a generated background — each glucose slot (fasted/breakfast/lunch/dinner) and BP slot (AM/PM) has its own independent day filter, so patients differ in _both_ how many days they log and how many slots they complete per day) covering normal/high severity for both glucose and blood pressure; patient C has a custom per-patient threshold. An access grant gives one doctor visibility into every seeded patient. Every account is loggable-in via the normal magic-link flow — see [Local login without real email](#local-login-without-real-email) below.

| Role              | Email                       | Scenario                                                                                                                                                                                                     |
| ----------------- | --------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Doctor (hospital) | `doctor.hospital@seed.test` | Single public institution affiliation; has access to all seeded patients                                                                                                                                     |
| Doctor (practice) | `doctor.practice@seed.test` | Private-practice-only affiliation (no institution)                                                                                                                                                           |
| Doctor (hybrid)   | `doctor.hybrid@seed.test`   | Both a public institution affiliation and a named private practice                                                                                                                                           |
| Doctor (solo)     | `doctor.solo@seed.test`     | No affiliations at all                                                                                                                                                                                       |
